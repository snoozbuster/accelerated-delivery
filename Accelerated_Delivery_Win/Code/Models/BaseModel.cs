...
namespace Accelerated_Delivery_Win
{
    public class BaseModel : ISpaceObject
    {
        ...
        protected Model internalModel;
        public Model Model { get { return internalModel; } }
        ...
        public BaseModel(Model model, bool glass, bool? mobile, Vector3 origin)
        {
            UseCustomAlpha = false;
            ...
            if(mobile.HasValue)
            {
                if(mobile.Value)
                {
                    ...
                    if(Program.Game.LoadingScreen == null || model == Program.Game.LoadingScreen.loader.boxModel)
                    {
        ...
        protected BaseModel(Model m, bool glass, Vector3 origin)
        {
            internalModel = m;
            RenderAsGlass = glass;
            Origin = origin;
            ...
            if(otherEntity != null)
            {
                if(otherEntity.Tag is Box)
                {
                    if(UsesLaserSound)
                    {
                        ...
                        MediaSystem.PlaySoundEffect(MediaSystem.SFXOptions.Laser);
                    }
        ...
        public static explicit operator BaseModel(Model m)
        {
            if(m == null)
                return null;
