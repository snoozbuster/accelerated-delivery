...
namespace Accelerated_Delivery_Win
{
    public class MyCamera
    {
        ...
        {
            if(Program.Game.State == BaseGame.GameState.Results)
                return;
            ...
            {
#endif
                if(!Program.Game.Manager.CurrentSaveWindowsOptions.SwapCamera)
                {
                    if(Input.KeyboardState.IsKeyDown(Program.Game.Manager.CurrentSaveWindowsOptions.CameraDownKey) ||
                    ...
                    if(Input.KeyboardState.IsKeyDown(Program.Game.Manager.CurrentSaveWindowsOptions.CameraUpKey) ||
                ...
                if(Input.KeyboardState.IsKeyDown(Program.Game.Manager.CurrentSaveWindowsOptions.CameraLeftKey) ||
                ...
                if(Input.KeyboardState.IsKeyDown(Program.Game.Manager.CurrentSaveWindowsOptions.CameraRightKey) ||
                   Input.CurrentPad.IsButtonDown(Buttons.LeftThumbstickRight) || Input.CurrentPad.IsButtonDown(Buttons.RightThumbstickRight))
                    this.HorizontalAngle += gameTime.ElapsedGameTime.Milliseconds * MathHelper.ToRadians(0.1f);

                if(Input.KeyboardState.IsKeyDown(Program.Game.Manager.CurrentSaveWindowsOptions.CameraZoomPlusKey) ||
                    (Program.Game.Manager.CurrentSaveWindowsOptions.CameraZoomPlusKey == Keys.Add && Input.KeyboardState.IsKeyDown(Keys.OemPlus)) ||
                   Input.CurrentPad.IsButtonDown(Program.Game.Manager.CurrentSaveXboxOptions.CameraZoomPlusKey))
                    this.Zoom -= (float)gameTime.ElapsedGameTime.TotalSeconds * 50f;

#if DEBUG
                if(Program.Game.CurrentLevel != null && Program.Game.CurrentLevel.spawnlevel == 0)
#endif
                if(Input.KeyboardState.IsKeyDown(Program.Game.Manager.CurrentSaveWindowsOptions.CameraZoomMinusKey) ||
                    (Program.Game.Manager.CurrentSaveWindowsOptions.CameraZoomMinusKey == Keys.Subtract && Input.KeyboardState.IsKeyDown(Keys.OemMinus)) ||
                   Input.CurrentPad.IsButtonDown(Program.Game.Manager.CurrentSaveXboxOptions.CameraZoomMinusKey))
                    this.Zoom += (float)gameTime.ElapsedGameTime.TotalSeconds * 50f;
#if DEBUG
            }
#endif
