...
namespace Accelerated_Delivery_Win
{
    public class Sprite
    {
        public Vector2 Scale { get { return Program.Game.TextureScaleFactor; } }
        ...
        public Sprite(Vector2 upLeft, Vector2 heightWidth)
        {
            ...
            relativeScreenPosition = new Vector2(upperLeft.X / Program.Game.Width, upperLeft.Y / Program.Game.Height);

            Program.Game.GDM.DeviceReset += rebuildCenters;
        }
        ...
        /// texture as well.
        /// </summary>
        /// <param name="tex">The texture. Pass as a ref so that multiple SuperTextors can use
        /// the same compiled texture without using up more space.</param>
        ...
        public Sprite(ref Texture2D tex, Vector2 position, Rectangle? renderBox, RenderPoint point)
        {
            Texture = tex;
            if(renderBox.HasValue)
                TargetArea = renderBox.Value;
            else
                TargetArea = new Rectangle(0, 0, tex.Width, tex.Height);
            Point = point;
        ...
        public void Draw(Color tint)
        {
            ...
            {
                if(Point == RenderPoint.UpLeft)
                    Program.Game.SpriteBatch.Draw(Texture, upperLeft, TargetArea, tint, 0, Vector2.Zero, DrawUnscaled ? Vector2.One : Scale, SpriteEffects.None, 0);
                else
                    Program.Game.SpriteBatch.Draw(Texture, center, TargetArea, tint, 0.0f, relativeOrigin, DrawUnscaled ? Vector2.One : Scale, SpriteEffects.None, 0);
            }
        }
        /// <summary>
        ...
        public void MoveTo(Vector2 newPosition, float timeInSeconds)
        {
            ...
            speed = distance * Program.Game.Space.TimeStepSettings.TimeStepDuration / timeInSeconds;
            Moving = true;
        ...
        private void update()
        {
            ...
            if(timeInSeconds != 0)
            {
                timer += Program.Game.Space.TimeStepSettings.TimeStepDuration;
        ...
        internal void ForceMoveUpdate(GameTime gameTime)
        {
            if(!Moving)
                return;
            upperLeft += speed * (float)(gameTime.ElapsedGameTime.TotalSeconds / Program.Game.Space.TimeStepSettings.TimeStepDuration);
            center += speed;
            lowerRight += speed;
        ...
        private void rebuildCenters(object caller, EventArgs e)
        {
            ...
            distance *= Program.Game.TextureScaleFactor;

            upperLeft = new Vector2(Program.Game.Width, Program.Game.Height) * relativeScreenPosition;
            lowerRight = new Vector2(upperLeft.X + Width, upperLeft.Y + Height);
            center = new Vector2((upperLeft.X + lowerRight.X) * 0.5f, (upperLeft.Y + lowerRight.Y) * 0.5f);
