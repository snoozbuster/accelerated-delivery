    ...
    public class ClampedRotationMachine : OperationalMachine
    {
            ...
            : base(machineNo, soundIndex, objects)
        {
            ...
            if(modelList.Count > 1)
                foreach(BaseModel m in modelList)
                {
                    ...
                    //motorList.Add(new RotateMotor(m, radiansToRotate) { Time = Program.Game.Space.TimeStepSettings.TimeStepDuration });
            ...
            foreach(Tube t in tubeList)
                ...
                //motorList.Add(new RotateMotor(t, radiansToRotate, t.LocalOffset) { Time = Program.Game.Space.TimeStepSettings.TimeStepDuration });
        }
        ...
        protected override void updateVelocities()
        {
            ...
            if(rotating)
            {
                ...
                //    if(atOrigin)
                //        j.Motor.Settings.Servo.Goal += Math.Min(angle / timeStep * Program.Game.Space.TimeStepSettings.TimeStepDuration, j.Limit.MaximumAngle);
                //    else
                //        j.Motor.Settings.Servo.Goal += Math.Max(-angle / timeStep * Program.Game.Space.TimeStepSettings.TimeStepDuration, j.Limit.MinimumAngle);
                //if(atOrigin)
                //    baseJoint.Motor.Settings.Servo.Goal += Math.Min(angle / timeStep * Program.Game.Space.TimeStepSettings.TimeStepDuration, baseJoint.Limit.MaximumAngle);
                //else
                //    baseJoint.Motor.Settings.Servo.Goal += Math.Max(-angle / timeStep * Program.Game.Space.TimeStepSettings.TimeStepDuration, baseJoint.Limit.MinimumAngle);
            }
            //else
            //    resetVelocities();
            ...
            //{
                ...
                //t.Ent.AngularVelocity = GetAngularVelocity(t.Ent.Orientation, rotation, Program.Game.Space.TimeStepSettings.TimeStepDuration);

                //t.Ent.LinearVelocity = (translation - t.Ent.Position) / (float)Program.Game.Space.TimeStepSettings.TimeStepDuration;
            //}
        }
