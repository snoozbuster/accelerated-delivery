    ...
    public class TranslateMachine : OperationalMachine
    {
        ...
        public bool Kinetic
        {
            set
            {
                ...
                if(baseJoint.Solver != null)
                    Program.Game.Space.Remove(baseJoint);
            }
        }
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if(translating)
            {
                ...
                {
                    if(atOrigin)
                        if(pathTime >= preDelayTime && preDelayTime != 0 && pathTime <= preDelayTime + Program.Game.Space.TimeStepSettings.TimeStepDuration)
                            baseJoint.Motor.Settings.Servo.Goal = targetPosition.Length();
                    if(!atOrigin)
                        if(pathTime >= postDelayTime && postDelayTime != 0 && pathTime <= postDelayTime + Program.Game.Space.TimeStepSettings.TimeStepDuration)
                            baseJoint.Motor.Settings.Servo.Goal = 0;
                }
