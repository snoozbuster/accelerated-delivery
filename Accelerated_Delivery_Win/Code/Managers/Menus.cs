    ...
    public static class MenuHandler
    {
        ...
        private class MediaMenu : Menu
        {
            ...
            public override void Draw(GameTime gameTime)
            {
                ...
                if(state != MenuState.Normal || backgroundTint.A > 0)
                {
                    ...
                    else if(Input.CheckMouseWithinCoords(sliderPos, sliderPos + new Vector2(sliderRenderRect.Width, sliderRenderRect.Height) * RenderingDevice.TextureScaleFactor))
                ...
                if(Input.ControlScheme == ControlScheme.Keyboard)
                    SymbolWriter.WriteKeyboardIcon(Input.WindowsOptions.MusicKey,
                    ...
                    SymbolWriter.WriteXboxIcon(Input.XboxOptions.MusicKey,
                        (mainRenderPos + new Vector2(mainRenderRect.Width / 2.0f + 25, mainRenderRect.Height + 15)) * RenderingDevice.TextureScaleFactor - dim + firstPart, true);
            ...
            public override void Update(GameTime gameTime)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        #region slider
                        if(Input.CheckMouseWithinCoords(sliderPos * RenderingDevice.TextureScaleFactor, (sliderPos + new Vector2(sliderRenderRect.Width, sliderRenderRect.Height)) * RenderingDevice.TextureScaleFactor) &&
                            !Input.CheckMouseLastFrameWithinCoords(sliderPos * RenderingDevice.TextureScaleFactor, (sliderPos + new Vector2(sliderRenderRect.Width, sliderRenderRect.Height)) * RenderingDevice.TextureScaleFactor))
                            MediaSystem.PlaySoundEffect(SFXOptions.Button_Rollover);
                        ...
                        {
                            if(Input.MouseState.Y > sliderBaseRenderPos.Y * RenderingDevice.TextureScaleFactor.Y &&
                                Input.MouseState.Y < (sliderBaseRenderPos.Y + sliderSlideDistance) * RenderingDevice.TextureScaleFactor.Y)
                                sliderPos.Y = Input.MouseState.Y;
                            else if(Input.MouseState.Y < sliderBaseRenderPos.Y * RenderingDevice.TextureScaleFactor.Y)
                                sliderPos.Y = sliderBaseRenderPos.Y * RenderingDevice.TextureScaleFactor.Y;
                            else
                                sliderPos.Y = (sliderBaseRenderPos.Y + sliderSlideDistance) * RenderingDevice.TextureScaleFactor.Y;
                        ...
                        if(!MouseTempDisabled)
                        {
                            if(!sliding && !holdingEnter)
                            {
                                ...
                                if(Input.CheckMouseWithinCoords(upperLeftBlack, lowerRightBlack) && Input.CheckMouseJustClicked(Program.Game.IsActive))
                                {
                                    holdingEnter = true;
                                    MediaSystem.PlaySoundEffect(SFXOptions.Button_Press);
                                }
        ...
        private class AchievementMenu : Menu
        {
            ...
            public override void Draw(GameTime gameTime)
            {
                ...
                if(sliding)
                    RenderingDevice.SpriteBatch.Draw(background, sliderPos + basePos * RenderingDevice.TextureScaleFactor, sliderRenderRect, downTint,
                        0, Vector2.Zero, RenderingDevice.TextureScaleFactor, SpriteEffects.None, 0);
                else if(Input.CheckMouseWithinCoords(sliderPos + basePos * RenderingDevice.TextureScaleFactor, sliderPos + (basePos + new Vector2(sliderRenderRect.Width, sliderRenderRect.Height)) * RenderingDevice.TextureScaleFactor))
            ...
            public override void Update(GameTime gameTime)
            {
                ...
                #region slider
                if(!sliding && Input.CheckMouseWithinCoords(sliderPos + basePos * RenderingDevice.TextureScaleFactor, sliderPos + (basePos + new Vector2(sliderRenderRect.Width, sliderRenderRect.Height)) * RenderingDevice.TextureScaleFactor) &&
                    !Input.CheckMouseLastFrameWithinCoords(sliderPos + basePos * RenderingDevice.TextureScaleFactor, sliderPos + (basePos + new Vector2(sliderRenderRect.Width, sliderRenderRect.Height)) * RenderingDevice.TextureScaleFactor))
                    MediaSystem.PlaySoundEffect(SFXOptions.Button_Rollover);
                if(!sliding && Input.CheckMouseWithinCoords(sliderPos + basePos * RenderingDevice.TextureScaleFactor, sliderPos + (basePos + new Vector2(sliderRenderRect.Width, sliderRenderRect.Height)) * RenderingDevice.TextureScaleFactor) &&
                    Input.MouseState.LeftButton == ButtonState.Pressed)
                {
                ...
                {
                    if(Input.MouseState.Y > (sliderBaseRenderPos.Y + basePos.Y) * RenderingDevice.TextureScaleFactor.Y &&
                        Input.MouseState.Y < (sliderBaseRenderPos.Y + basePos.Y + sliderSlideDistance) * RenderingDevice.TextureScaleFactor.Y)
                        sliderPos.Y = Input.MouseState.Y - basePos.Y * RenderingDevice.TextureScaleFactor.Y - sliderRenderRect.Height * 0.5f * RenderingDevice.TextureScaleFactor.Y;
                    else if(Input.MouseState.Y < (sliderBaseRenderPos.Y + basePos.Y) * RenderingDevice.TextureScaleFactor.Y)
                        sliderPos.Y = (sliderBaseRenderPos.Y) * RenderingDevice.TextureScaleFactor.Y;
                    else
                        sliderPos.Y = (sliderBaseRenderPos.Y + sliderSlideDistance) * RenderingDevice.TextureScaleFactor.Y;
